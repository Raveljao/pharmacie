...
export default function CataloguePage() {
    ...
    const filteredProducts = products.filter((product: Product) => {
        const matchesSearch = searchQuery === '' ||
            ...
            product.nom_mg.toLowerCase().includes(searchQuery.toLowerCase()) ||
            ...
            <div className="w-full px-6 lg:px-12 xl:px-20 max-w-[1800px] mx-auto">
                ...
                <div className="space-y-8 mb-12">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer select-none">
                        <div className={`w-6 h-6 flex items-center justify-center border-2 transition-colors rounded-sm ${showAvailableOnly ? 'bg-blue-600 border-blue-600' : 'bg-white border-slate-300'}`}>
                            {showAvailableOnly && (
                        ...
                        <input
                            ...
                            className="hidden"
                        />
                        <span className="text-sm font-bold uppercase tracking-wide text-slate-600">{t('showAvailable')}</span>
                    </label>
                </div>
                ...
                {filteredProducts.length === 0 && (
                    <div className="text-center py-20 bg-slate-50 border-2 border-slate-100 rounded-sm">
                        ...
                        <p className="text-slate-500 font-light tracking-wide uppercase text-sm mb-8">Aucun produit trouvé</p>
                        <button
                            onClick={() => {
                                setSearchQuery('');
                                setSelectedCategory('');
